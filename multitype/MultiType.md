...
> 本文引用me.drakeet.multitype，在此基础上略微修改，用于公司内部app
...
```groovy
dependencies {
    compile 'cn.daily.android:multitype:0.0.1-SNAPSHOT'
}
```
...
**本版本会根据内部需求做一定的修改，如果有其他问题，可以参考[drakeet/MultiType](https://github.com/drakeet/MultiType)**
...
- **Q：如何在 `ItemViewBinder` 中获取到 item position？**
  ...
  A: 在你的 `ItemViewBinder` 子类里调用 `getPosition(holder)` 方法即可。另外，`ItemViewBinder` 还提供了 `getAdapter()` 或许也是很多人想要的，比如调用 adapter 进行 notify 刷新视图等。
